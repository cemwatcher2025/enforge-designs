...
- KIM Vision uses a field-note style observation prompt and cleans obvious model prompt echoes before saving deep observations, so instruction fragments do not pollute the memory log. Clothing, facial expressions, and stable room descriptions are discouraged unless unmistakable.
