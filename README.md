...
- KIM Vision now separates motion into object-level, posture, room-level, and settled scopes. Object-level motion has a short micro-assessment lane so subtle cup/phone/tool movements can be checked without making every posture shift trigger a full caption.
...
- KIM Vision V2 includes a lazy-loaded local COCO-SSD detector through TensorFlow.js/WebGL. The detector recognizes useful structured objects such as person, dog, chair, cell phone, cup, and bottle, then feeds those facts into scene memory without waiting for Moondream captions.
- KIM Vision V2 stores a longer local sensor-event history for calibration. The Situational Awareness panel now keeps up to 60 detector/sensor events, displays the latest 16, and uses lighter duplicate suppression for detector readings so performance patterns remain visible.
- KIM Vision has a local debug packet exporter. Brandon can add a short truth note, copy or download a JSON packet, and send Codex the recent scene memory, detector/sensor events, model assessments, active settings, and status without sharing screenshots or video.
- KIM Vision debug packets include `appBuild.sha` and `appBuild.time`, and the dashboard header shows a build badge. This makes browser-cache/version drift visible when testing newly deployed awareness logic.
- KIM Vision motion scoring is local-aware: it combines whole-frame change, hot-zone movement, and active changed pixels so distant walking or doorway movement is less likely to vanish into a low global motion percentage.
- KIM Ambient Awareness Layer 1 is now implemented in the browser-local camera loop. Automatic sampling uses a strict change gate with configurable seconds-based sampling, configurable meaningful-change threshold, dwell-time tracking, and a "last interesting frame" baseline. Frames below the gate are discarded before detector or Moondream/GPU captioning. Manual Analyze still bypasses the gate for direct inspection.
- KIM Ambient Awareness Layer 1 uses a plain pixel-change score for the gate, separate from the amplified motion score used for responsiveness. Passing the gate does not bypass the normal model cooldown; detector/sensor events can update quietly while Moondream/GPU captioning remains throttled.
- KIM Ambient Awareness Layer 1 gate scoring is block-based so small pixel shimmer, camera noise, auto-exposure, and fan flicker are less likely to look like major scene changes. New Analyze sessions reset the interesting-frame baseline, and debug packets focus on the current session instead of mixing in older localStorage history.
- KIM Ambient Awareness has a 60-second startup quiet window. After Analyze starts, first-minute gate passes can update sensors and memory quietly, but they do not trigger Moondream/GPU captions unless both structural change and motion are large. This prevents baseline settling from producing immediate "man sitting" narration.
- KIM Ambient Awareness uses discarded calm frames as settling evidence. If scene memory says the person is moving, holding an object, or standing, a later below-gate calm frame can update memory back toward seated/settled without running detector or Moondream/GPU captioning.
- KIM Vision V2 uses detector posture evidence when the room calms down. If the detector sees a person and chair during low motion after a movement event, scene memory can settle to sitting without waiting for another deep caption.
- KIM Vision V2 separates learned motion notices from deep caption triggers. Tiny movement can still update memory and appear in sensor logs, but automatic GPU/Moondream calls require at least 7% motion, a meaningful light change, a strong learned-motion spike, or a structural scene-change score of 12%+.
- KIM Vision V2 does not let every motion event overwrite posture. Baseline-breaking motion can be logged while preserving `sitting` or `object_in_hand`; scene memory only switches to `moving` for strong motion or wide-region movement.
- KIM Vision V2 adds motion scope classification. Every sensor event, assessment, scene memory summary, and debug packet can now distinguish `fine_object_motion`, `posture_motion`, `room_motion`, and `settled` so close cup/phone/fork movement stays visible without being treated as a whole-room event.
- KIM Vision V2 gives recent local detector object findings priority over stale caption details. If the detector recently saw a cup, phone, fork, bottle, or similar held object and the caption model reports clothing/paint/room details instead, KIM keeps the object-change context.
...
- KIM Vision V2 treats posture as sensor-confirmed state, not caption-only truth. Manual captions can be logged, but they no longer overwrite scene memory to "standing" unless recent live motion or motion region evidence supports that posture change.
- KIM Vision V2 does not treat lower-region-only movement as enough evidence for standing. This prevents sit-down motion from being misread as a new standing state when the caption model guesses wrong.
- KIM Vision V2 now treats post-motion settling as useful posture evidence. When scene memory says standing, moving, or object-in-hand and the live sensor drops back to baseline, KIM can mark the state as likely seated/settled instead of staying stuck on standing.
...
- KIM Vision strips additional caption artifacts such as "blurry photo", "bald head", "shaved head", and couch/background guesses when they are likely model phrasing noise instead of meaningful context.
